...
class QuizController:
    ...
    def create_quiz(
        self, payload: CreateQuizRequest, background_tasks: BackgroundTasks
    ) -> SessionResponse:
        """
        ...
        try:
            ...
            description = (
                payload.description
                or f"Quiz for {payload.topic}. Difficulty: {payload.difficulty.value}."
            )
    ...
    def list_sessions(self, status: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        for session in orchestrator_sessions:
            ...
            if not question_count or question_count is None:
                ...
                try:
                    self.orchestrator.session_manager.save_session(
                        session["session_id"], session
                    )
                except Exception as e:
                    logger.warning(
                        f"Failed to save question_count for session {session['session_id']}: {e}"
                    )
    ...
    def retry_session(
        ...
        try:
            ...
            return SessionResponse(
                sessionId=session_id, message=f"Resuming quiz session: {session_id}"
            )
        except ValueError:
            raise HTTPException(status_code=404, detail="Session not found")
    ...
    def generate_topic(
        ...
        try:
            description = (
                f"Quiz for {payload.topic}. Difficulty: {payload.difficulty.value}."
            )
    ...
    def _validate_question(self, question: Dict[str, Any], index: int) -> List[str]:
        ...
        if len(options) < 2:
            errors.append(
                f"{prefix}: At least 2 options required (found {len(options)})"
            )
        ...
        if difficulty not in ["easy", "medium", "hard"]:
            errors.append(
                f"{prefix}: Invalid difficulty '{difficulty}' (must be easy/medium/hard)"
            )
    ...
    def upload_quiz(self, payload: UploadQuizRequest) -> SimpleStatus:
        ...
        try:
            quiz_dict = (
                ...
                if hasattr(payload.quiz, "model_dump")
                else payload.quiz
            )
            ...
            if not validation.ok:
                return SimpleStatus(
                    ok=False, message=f"Validation failed: {validation.message}"
                )
            ...
            if response.status_code in [200, 201]:
                ...
                return SimpleStatus(
                    ok=False, message=f"Upload failed: HTTP {response.status_code}"
                )
