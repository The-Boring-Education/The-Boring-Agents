...
async def create_sheet(
    payload: CreateSheetRequest, background_tasks: BackgroundTasks, request: Request
):
    ...
    try:
        ...
        log_action(
            request, "create_sheet", status="success", session_id=result.sessionId
        )
        return result
...
async def retry_session(
    ...
    try:
        ...
        log_action(
            request, "retry_session", level="ERROR", session_id=session_id, error=str(e)
        )
        raise
...
async def update_question(
    session_id: str, question_id: str, updates: dict, request: Request
):
    """Update a generated question in a session."""
    log_action(
        request, "update_question", session_id=session_id, question_id=question_id
    )
    try:
        if not updates:
...
async def update_question_alias(
    session_id: str, question_id: str, updates: dict, request: Request
):
