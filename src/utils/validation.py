...
class InterviewQuestionValidator:
    ...
    def validate_question_data(question_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not frequency or frequency not in [f.value for f in FrequencyType]:
            warnings.append(
                f"Invalid frequency: {frequency}. Setting to 'Asked Frequently'"
            )
        ...
        if not company_types:
            ...
            if any(
                word in (question_text + title_text)
                for word in ["basic", "fundamental", "syntax", "variable", "function"]
            ):
                question_data["companyTypes"] = ["Startup", "MidSize"]
            elif any(
                word in (question_text + title_text)
                for word in ["advanced", "complex", "algorithm", "system design"]
            ):
            ...
            warnings.append(
                f"Auto-fixed empty companyTypes to: {question_data['companyTypes']}"
            )
        else:
            valid_company_types = [ct.value for ct in CompanyType]
            invalid_types = [
                ct for ct in company_types if ct not in valid_company_types
            ]
            if invalid_types:
                warnings.append(
                    f"Invalid company types: {invalid_types}. Removing invalid types."
                )
                question_data["companyTypes"] = [
                    ct for ct in company_types if ct in valid_company_types
                ]
        ...
        if answer:
            question_data["answer"] = InterviewQuestionValidator._clean_answer_format(
                answer
            )
    ...
    def validate_sheet_data(sheet_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not questions:
            ...
            for i, question in enumerate(questions):
                validation_result = InterviewQuestionValidator.validate_question_data(
                    question
                )
                if validation_result["is_valid"]:
                    ...
                    errors.extend(
                        [
                            f"Question {i + 1}: {error}"
                            for error in validation_result["errors"]
                        ]
                    )
                    warnings.extend(
                        [
                            f"Question {i + 1}: {warning}"
        ...
        for i, question in enumerate(questions):
            ...
            if not company_types:
                return {
                    ...
                    "errors": [
                        f"Question {i + 1}: companyTypes cannot be empty. This indicates the AI failed to properly analyze the question."
                    ],
                    "warnings": validation_result["warnings"],
                }
            ...
            if (
                priority == "Medium"
                and frequency == "Asked Frequently"
                and len(company_types) <= 1
            ):
                return {
                    "can_publish": False,
                    "reason": f"Question {i + 1} has generic default values - AI needs to analyze this question more carefully",
                    "errors": [
                        f"Question {i + 1}: Generic defaults detected. AI should provide specific analysis."
                    ],
                    "warnings": validation_result["warnings"],
                }
