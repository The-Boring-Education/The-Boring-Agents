...
class EnvironmentManager:
    ...
    def _load_environment(self) -> None:
        ...
        if loaded_files:
            logger.info(f"Environment loaded from {len(loaded_files)} file(s): {', '.join(loaded_files)}")
        else:
            logger.warning("No .env files found. Using system environment variables and defaults.")
